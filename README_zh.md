...
- **持久化存储**：
  - **按二进制保存**：切片列表会根据二进制文件的 MD5 自动保存和加载。
  - **路径记忆**：记住上次用于导入 `.seg` 文件的目录。
- **数据库切片 (IDA 9.1+)**：
...
- **段导入 (IDA 9.1+)**：
  - 将之前导出的 `.seg` 文件导入到现有数据库中。
  - 智能处理重叠：可选择覆盖现有数据或在空隙处创建新段。

## 要求

- **IDA Pro 9.1+**：“Slice and Create IDA Database”和“Import .seg files”功能需要（需要 `ida_domain` API）。
...
3. **检查与编辑**：
   - 双击表格中的条目以修改其属性。
   - 右键单击条目以将其删除。
   - 列表会自动保存到 `idaslicer_config.json`。
4. **导出与导入**：
   - **Slice and Create IDA Database**：生成一个新的 `.i64` 文件，重新创建段并填充数据。
   - **Save segments to .seg files**：将每个条目的原始字节转储到 `.seg` 文件中。
   - **Import .seg files**：将 `.seg` 文件重新加载到数据库中，并提供重叠段的冲突解决选项。
...
### 段导入
使用 `ida_domain` API 修改段和字节。它从 `.seg` 文件名中解析元数据，检测与现有段的重叠，并提供覆盖数据或通过创建新段来填补空隙的选项。
